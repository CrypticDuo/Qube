...
if (cluster.isMaster) {
  ...
  app.use(session({ store: new RedisStore({host: 'localhost', port: 6379}), secret: 'my precious' }))
  ...
  router.use(function(req, res, next) {
    console.log("Worker " + cluster.worker.id + ' : ', req.protocol + '://' + req.get('host') + req.originalUrl);
    next();
  });
  /*
  GET     /playlists
  ...
  */
  ...
  console.log("Listening on port localhost:" + app.get('port') + " reporting from worker #" +cluster.worker.id);
}
